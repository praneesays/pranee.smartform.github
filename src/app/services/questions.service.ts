...
export interface IIQuestionsRequest {
    complaintReport: IComplaintReporting;

    complaintData: IComplaintDetails;
...
const allQuestions: Question[] = [
    ...
    },
    {
        ...
        required: true,
        questionText:
            "If requested, would you be able to return the product using prepaid packaging?",
        options: [
            {
                name: "Yes"
            },
    ...
    },
    {
        ...
        required: true,
        questionText:
            "Did the device make any sounds when you pressed the button?",
        options: [
            {
    ...
    },
    {
        ...
        required: true,
        questionText:
            "To the best of your knowledge, was the dose administered despite the issue?",
        options: [
            {
                name: "Yes, completely",
                nextQuestionId: "who_administered"
...
export class QuestionsService {
    getNextQuestions(
        request: IIQuestionsRequest
    ): Observable<IQuestionsResponse> {
        if (request.answeredQuestions.length === 0) {
            const brandEntrypoint =
                request.complaintReport.brandAndForm &&
                allQuestionsMap.get(
                    brandEntrypoints[
                        request.complaintReport.brandAndForm.brandId
                    ]
                );
        ...
        const lastAnswer =
            request.answeredQuestions[request.answeredQuestions.length - 1];
        ...
        switch (lastQuestion.type) {
            ...
            case "multiple-choice": {
                nextQuestionId = lastQuestion.options.find(
                    (c) => c.name === lastAnswer.response
                )?.nextQuestionId;

                break;
            }
            case "image-map": {
                nextQuestionId = lastQuestion.areas.find(
                    (c) => c.value === lastAnswer.response
                )?.nextQuestionId;
        ...
        const nextQuestion = nextQuestionId
            ? allQuestionsMap.get(nextQuestionId)
            : undefined;
